@extends('layouts.app')
@section('page_title','Edit Nationality')
@section('content')
@section('css')
                     ...
                     <input type="hidden" value="{{$nationality['id']}}" name="id">
                        <div class="row m-2">
                            ...
                            <button class="btn btn-danger _effect--ripple waves-effect waves-light error_btn mx-2 bs-tooltip" data-bs-placement="bottom" title="Error" data-bs-toggle="modal" data-bs-target="#ErrorListModalCenter">Error</button>
                                <a href="{{route('nationality')}}" class="btn btn-info _effect--ripple waves-effect waves-light bs-tooltip" data-bs-placement="bottom" title="Back" type="submit">Back</a>
            ...
            <div class="col-lg-12 layout-spacing">
                <div class="statbox widget box box-shadow">
                    ...
                    <div class="widget-content widget-content-area">
                            ...
                            <div class="row g-3">
                            <div class="col-md-3">
                                <label for="validationDefault01" class="form-label">Nationality Id<span class="text-danger">
                                        *</span></label>
                                <input type="text" name='GMNAHNationalityId' id='GMNAHNationalityId' class='form-control threshold' maxlength="20" placeholder="Enter Nationality Name" style='border-color: rgb(102, 175, 233); outline: 0px' value="{{ old('GMNAHNationalityId', $nationality['GMNAHNationalityId'] ?? '') }}" readonly="readonly">
                            </div>
                            <div class="col-md-6">
                                <label for="GMNAHDesc1" class="form-label">Description 1<span class="text-danger"> *</span></label>
                                <input type="text" name='GMNAHDesc1' id='GMNAHDesc1' class='form-control threshold' maxlength="20" placeholder="Enter Description 1" style='border-color: rgb(102, 175, 233); outline: 0px' value="{{ old('GMNAHDesc1', $nationality['GMNAHDesc1'] ?? '') }}">

                            </div>
                            <div class="col-md-3">
                                <label for="validationDefault05" class="form-label">BI Desc<span class="text-danger">
                                        *</span></label>
                                <input type="text" name='GMNAHBiDesc' id='GMNAHBiDesc' class='form-control threshold' maxlength="20" placeholder="Enter Bi Desc" style='border-color: rgb(102, 175, 233); outline: 0px' value="{{ old('GMNAHBiDesc', $nationality['GMNAHBiDesc'] ?? '') }}">
                            </div>
                            <div class="col-md-12">
                                <label for="validationDefault03" class="form-label">Description 2</label>
                                <textarea name='GMNAHDesc2' id='GMNAHDesc2' class='form-control textarea' maxlength="200" placeholder="Enter Description 2" style='border-color: rgb(102, 175, 233); outline: 0px'>{{ old('GMNAHDesc2', $nationality['GMNAHDesc2'] ?? '') }}</textarea>
                            </div>
...
<script type="text/javascript">
    ...
    $("#AddForm").submit(function(e) {
        e.preventDefault();
        $('#GMNAHNationalityId,#GMNAHDesc1').removeClass('border border-danger');

        if ($('#GMNAHNationalityId').val() == '') {
            $('#GMNAHNationalityId').addClass('border border-danger');
        }
        if ($('#GMNAHDesc1').val() == '') {
            $('#GMNAHDesc1').addClass('border border-danger');
        }

        $('.form_error_list').html('');
        if ($('#GMNAHNationalityId').val() == '' || $('#GMNAHDesc1').val() == '') {
            if ($('#GMNAHNationalityId').val() == '') {
                $('.form_error_list').append('<p>Please Enter Nationality name !</p>');
            }
            if ($('#GMNAHDesc1').val() == '') {
                $('.form_error_list').append('<p>Please Enter Nationality Description 1 !</p>');
            }
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status == 'success') {
                        custom_notification({
                            text: 'Nationality Update successfully',
                            pos: 'bottom-right',
                            duration: 100000
                        })
                        window.location = "{{ url('nationality') }}";

                    }
                    if (response.status == 'error') {
                        // Bottom Right
                        custom_notification({
                            text: 'Nationality Master not save',
                            pos: 'bottom-right'
                        })
                    }
                }
