@extends('layouts.app')
@section('page_title','Edit Gender')
@section('content')
@section('css')
                     ...
                     <input type="hidden" value="{{$gender['id']}}" name="id">
                        <div class="row m-2">
                            ...
                            <button class="btn btn-danger _effect--ripple waves-effect waves-light error_btn mx-2 bs-tooltip" data-bs-placement="bottom" title="Error" data-bs-toggle="modal" data-bs-target="#ErrorListModalCenter">Error</button>
                                <a href="{{route('gender')}}" class="btn btn-info _effect--ripple waves-effect waves-light bs-tooltip" data-bs-placement="bottom" title="Back" type="submit">Back</a>
            ...
            <div class="col-lg-12 layout-spacing">
                <div class="statbox widget box box-shadow">
                    ...
                    <div class="widget-content widget-content-area">
                            ...
                            <div class="row g-3">
                            <div class="col-md-3">
                                <label for="validationDefault01" class="form-label">Gender Id<span class="text-danger">
                                        *</span></label>
                                <input type="text" name='GMGDHGenderId' id='GMGDHGenderId' class='form-control threshold' maxlength="20" placeholder="Enter Gender Name" style='border-color: rgb(102, 175, 233); outline: 0px' value="{{ old('GMGDHGenderId', $gender['GMGDHGenderId'] ?? '') }}" readonly="readonly">
                            </div>
                            <div class="col-md-6">
                                <label for="GMGDHDesc1" class="form-label">Description 1<span class="text-danger"> *</span></label>
                                <input type="text" name='GMGDHDesc1' id='GMGDHDesc1' class='form-control threshold' maxlength="20" placeholder="Enter Description 1" style='border-color: rgb(102, 175, 233); outline: 0px' value="{{ old('GMGDHDesc1', $gender['GMGDHDesc1'] ?? '') }}">

                            </div>
                            <div class="col-md-3">
                                <label for="validationDefault05" class="form-label">BI Desc<span class="text-danger">
                                        *</span></label>
                                <input type="text" name='GMGDHBiDesc' id='GMGDHBiDesc' class='form-control threshold' maxlength="20" placeholder="Enter Bi Desc" style='border-color: rgb(102, 175, 233); outline: 0px' value="{{ old('GMGDHBiDesc', $gender['GMGDHBiDesc'] ?? '') }}">
                            </div>
                            <div class="col-md-12">
                                <label for="validationDefault03" class="form-label">Description 2</label>
                                <textarea name='GMGDHDesc2' id='GMGDHDesc2' class='form-control textarea' maxlength="200" placeholder="Enter Description 2" style='border-color: rgb(102, 175, 233); outline: 0px'>{{ old('GMGDHDesc2', $gender['GMGDHDesc2'] ?? '') }}</textarea>
                            </div>
...
<script type="text/javascript">
    ...
    $("#AddForm").submit(function(e) {
        e.preventDefault();
        $('#GMGDHGenderId,#GMGDHDesc1').removeClass('border border-danger');

        if ($('#GMGDHGenderId').val() == '') {
            $('#GMGDHGenderId').addClass('border border-danger');
        }
        if ($('#GMGDHDesc1').val() == '') {
            $('#GMGDHDesc1').addClass('border border-danger');
        }

        $('.form_error_list').html('');
        if ($('#GMGDHGenderId').val() == '' || $('#GMGDHDesc1').val() == '') {
            if ($('#GMGDHGenderId').val() == '') {
                $('.form_error_list').append('<p>Please Enter Gender name !</p>');
            }
            if ($('#GMGDHDesc1').val() == '') {
                $('.form_error_list').append('<p>Please Enter Gender Description 1 !</p>');
            }
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status == 'success') {
                        custom_notification({
                            text: 'Gender Update successfully',
                            pos: 'bottom-right',
                            duration: 100000
                        })
                        window.location = "{{ url('gender') }}";

                    }
                    if (response.status == 'error') {
                        // Bottom Right
                        custom_notification({
                            text: 'Gender Master not save',
                            pos: 'bottom-right'
                        })
                    }
                }
