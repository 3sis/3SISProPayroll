@extends('layouts.app')
@section('page_title','Edit Salutation')
@section('content')
@section('css')
    ...
    <div class="container">
        <div class="row layout-top-spacing">
            <div id="basic" class="col-lg-12 layout-spacing">
                <div class="statbox widget box box-shadow">
                    ...
                    <form id='AddForm' method="post" autocomplete="off">
                     @csrf
                     <input type="hidden" value="{{$salutation['id']}}" name="id">
                        <div class="row m-2">
                            ...
                            <button class="btn btn-danger _effect--ripple waves-effect waves-light error_btn mx-2 bs-tooltip" data-bs-placement="bottom" title="Error" data-bs-toggle="modal" data-bs-target="#ErrorListModalCenter">Error</button>
                                <a href="{{route('salutation')}}" class="btn btn-info _effect--ripple waves-effect waves-light bs-tooltip" data-bs-placement="bottom" title="Back" type="submit">Back</a>
            ...
            <div class="col-lg-12 layout-spacing">
                <div class="statbox widget box box-shadow">
                    <div class="widget-header">
                    <h4>Edit Salutation</h4>
                    </div>
                    <div class="widget-content widget-content-area">

                            <div class="row g-3">
                            <div class="col-md-3">
                                <label for="validationDefault01" class="form-label">Salutation Id<span class="text-danger">
                                        *</span></label>
                                <input type="text" name='GMSLHSalutationId' id='GMSLHSalutationId' class='form-control threshold' maxlength="20" placeholder="Enter Salutation Name" style='border-color: rgb(102, 175, 233); outline: 0px' value="{{ old('GMSLHSalutationId', $salutation['GMSLHSalutationId'] ?? '') }}" readonly="readonly">
                            </div>
                            <div class="col-md-6">
                                <label for="GMSLHDesc1" class="form-label">Description 1<span class="text-danger"> *</span></label>
                                <input type="text" name='GMSLHDesc1' id='GMSLHDesc1' class='form-control threshold' maxlength="20" placeholder="Enter Description 1" style='border-color: rgb(102, 175, 233); outline: 0px' value="{{ old('GMSLHDesc1', $salutation['GMSLHDesc1'] ?? '') }}">

                            </div>
                            <div class="col-md-3">
                                <label for="validationDefault05" class="form-label">BI Desc<span class="text-danger">
                                        *</span></label>
                                <input type="text" name='GMSLHBiDesc' id='GMSLHBiDesc' class='form-control threshold' maxlength="20" placeholder="Enter Bi Desc" style='border-color: rgb(102, 175, 233); outline: 0px' value="{{ old('GMSLHBiDesc', $salutation['GMSLHBiDesc'] ?? '') }}">
                            </div>
                            <div class="col-md-12">
                                <label for="validationDefault03" class="form-label">Description 2</label>
                                <textarea name='GMSLHDesc2' id='GMSLHDesc2' class='form-control textarea' maxlength="200" placeholder="Enter Description 2" style='border-color: rgb(102, 175, 233); outline: 0px'>{{ old('GMSLHDesc2', $salutation['GMSLHDesc2'] ?? '') }}</textarea>
                            </div>
...
<script type="text/javascript">
    ...
    $("#AddForm").submit(function(e) {
        e.preventDefault();
        $('#GMSLHSalutationId,#GMSLHDesc1').removeClass('border border-danger');

        if ($('#GMSLHSalutationId').val() == '') {
            $('#GMSLHSalutationId').addClass('border border-danger');
        }
        if ($('#GMSLHDesc1').val() == '') {
            $('#GMSLHDesc1').addClass('border border-danger');
        }

        $('.form_error_list').html('');
        if ($('#GMSLHSalutationId').val() == '' || $('#GMSLHDesc1').val() == '') {
            if ($('#GMSLHSalutationId').val() == '') {
                $('.form_error_list').append('<p>Please Enter Salutation name !</p>');
            }
            if ($('#GMSLHDesc1').val() == '') {
                $('.form_error_list').append('<p>Please Enter Salutation Description 1 !</p>');
            }
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status == 'success') {
                        custom_notification({
                            text: 'Salutation Update successfully',
                            pos: 'bottom-right',
                            duration: 100000
                        })
                        window.location = "{{ url('salutation') }}";

                    }
                    if (response.status == 'error') {
                        // Bottom Right
                        custom_notification({
                            text: 'Salutation Master not save',
                            pos: 'bottom-right'
                        })
                    }
                }
