@extends('layouts.app')
@section('page_title','Edit Blood Group')
@section('content')
@section('css')
    ...
    <div class="container">
        <div class="row layout-top-spacing">
            <div id="basic" class="col-lg-12 layout-spacing">
                <div class="statbox widget box box-shadow">
                    ...
                    <form id='AddForm' method="post" autocomplete="off">
                     @csrf
                     <input type="hidden" value="{{$bloodGroup['id']}}" name="id">
                        <div class="row m-2">
                            ...
                            <button class="btn btn-danger _effect--ripple waves-effect waves-light error_btn mx-2 bs-tooltip" data-bs-placement="bottom" title="Error" data-bs-toggle="modal" data-bs-target="#ErrorListModalCenter">Error</button>
                                <a href="{{route('blood')}}" class="btn btn-info _effect--ripple waves-effect waves-light bs-tooltip" data-bs-placement="bottom" title="Back" type="submit">Back</a>
            ...
            <div class="col-lg-12 layout-spacing">
                <div class="statbox widget box box-shadow">
                    <div class="widget-header">
                    <h4>Edit Blood Group</h4>
                    </div>
                    <div class="widget-content widget-content-area">

                            <div class="row g-3">
                            <div class="col-md-3">
                                <label for="validationDefault01" class="form-label">Blood Group Id<span class="text-danger">
                                        *</span></label>
                                <input type="text" name='GMBGHBloodGroupId' id='GMBGHBloodGroupId' class='form-control threshold' maxlength="20" placeholder="Enter Blood Group Name" style='border-color: rgb(102, 175, 233); outline: 0px' value="{{ old('GMBGHBloodGroupId', $bloodGroup['GMBGHBloodGroupId'] ?? '') }}" readonly="readonly">
                            </div>
                            <div class="col-md-6">
                                <label for="GMBGHDesc1" class="form-label">Description 1<span class="text-danger"> *</span></label>
                                <input type="text" name='GMBGHDesc1' id='GMBGHDesc1' class='form-control threshold' maxlength="20" placeholder="Enter Description 1" style='border-color: rgb(102, 175, 233); outline: 0px' value="{{ old('GMBGHDesc1', $bloodGroup['GMBGHDesc1'] ?? '') }}">

                            </div>
                            <div class="col-md-3">
                                <label for="validationDefault05" class="form-label">BI Desc<span class="text-danger">
                                        *</span></label>
                                <input type="text" name='GMBGHBiDesc' id='GMBGHBiDesc' class='form-control threshold' maxlength="20" placeholder="Enter Bi Desc" style='border-color: rgb(102, 175, 233); outline: 0px' value="{{ old('GMBGHBiDesc', $bloodGroup['GMBGHBiDesc'] ?? '') }}">
                            </div>
                            <div class="col-md-12">
                                <label for="validationDefault03" class="form-label">Description 2</label>
                                <textarea name='GMBGHDesc2' id='GMBGHDesc2' class='form-control textarea' maxlength="200" placeholder="Enter Description 2" style='border-color: rgb(102, 175, 233); outline: 0px'>{{ old('GMBGHDesc2', $bloodGroup['GMBGHDesc2'] ?? '') }}</textarea>
                            </div>
...
<script type="text/javascript">
    ...
    $("#AddForm").submit(function(e) {
        e.preventDefault();
        $('#GMBGHBloodGroupId,#GMBGHDesc1').removeClass('border border-danger');

        if ($('#GMBGHBloodGroupId').val() == '') {
            $('#GMBGHBloodGroupId').addClass('border border-danger');
        }
        if ($('#GMBGHDesc1').val() == '') {
            $('#GMBGHDesc1').addClass('border border-danger');
        }

        $('.form_error_list').html('');
        if ($('#GMBGHBloodGroupId').val() == '' || $('#GMBGHDesc1').val() == '') {
            if ($('#GMBGHBloodGroupId').val() == '') {
                $('.form_error_list').append('<p>Please Enter Blood Group name !</p>');
            }
            if ($('#GMBGHDesc1').val() == '') {
                $('.form_error_list').append('<p>Please Enter Blood Group Description 1 !</p>');
            }
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status == 'success') {
                        custom_notification({
                            text: 'Blood Group Update successfully',
                            pos: 'bottom-right',
                            duration: 100000
                        })
                        window.location = "{{ url('blood') }}";

                    }
                    if (response.status == 'error') {
                        // Bottom Right
                        custom_notification({
                            text: 'Blood Group Master not save',
                            pos: 'bottom-right'
                        })
                    }
                }
