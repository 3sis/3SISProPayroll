@extends('layouts.app')
@section('page_title', 'Blood Group')
@section('content')
@section('css')
...
<div class="container">
    <div class="row mt-2">
        <div class="col-xl-6 col-md-6 col-sm-6 col-6 d-grid gap-1 d-md-block">
            ...
            <nav class="breadcrumb-style-five  mb-3" aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active"><a href="#">Blood Group Master</a></li>
                </ol>
            </nav>
        ...
        <div class="col-xl-6 col-md-6 col-sm-6 col-6 d-grid d-md-flex justify-content-md-end">
            ...
            <a href="{{ route('add_blood') }}" class="btn btn-success me-4 btn-sm mx-1 bs-tooltip" data-bs-placement="bottom" title="Add">
            ...
            <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalCenterTitle">Restore Blood Group</h5>
            ...
            <div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
                <div class="widget-content widget-content-area br-8">
                    <table id="zero-config" class="table dt-table-hover" style="width:100%">
                        ...
                        <tbody>
                            @foreach ($bloodGroup_list as $row)
                            {{-- {{ $row }} --}}
                            <tr>
                                <td>{{ $row->GMBGHBloodGroupId }}</td>
                                <td>{{ $row->GMBGHDesc1 }}</td>
                                <td>{{ $row->GMBGHBiDesc }}</td>
                                <td>{{ $row->GMBGHUser }}</td>
                                <td>
                                    <!-- Edit -->
                                    <a href="edit_blood/{{ Crypt::encryptString($row->id) }}"> <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit-2 bs-tooltip" data-bs-placement="bottom" title="Edit">
...
<script>
    ...
    $(document).on('click', '.delete', function() {
        ...
        $deleteMessage3SIS = fnConfirmationMsg('Delete', 'Blood Group', lid, desc);
        $successMessage3SIS = fnSuccessMsg('Deleted', 'Blood Group', lid, desc);
    ...
    $(document).on('click', '.restore', function() {
        ...
        var lid = $(this).closest("tr").find("td:eq(0)").text();
        var desc = $(this).closest("tr").find("td:eq(1)").text();
        $restoreMessage3SIS = fnConfirmationMsg('Restore', 'Blood Group', lid, desc);
        Swal.fire({
