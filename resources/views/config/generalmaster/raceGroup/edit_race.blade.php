@extends('layouts.app')
@section('page_title','Edit Race Group')
@section('content')
@section('css')
    ...
    <div class="container">
        <div class="row layout-top-spacing">
            <div id="basic" class="col-lg-12 layout-spacing">
                <div class="statbox widget box box-shadow">
                    ...
                    <form id='AddForm' method="post" autocomplete="off">
                     @csrf
                     <input type="hidden" value="{{$raceGroup['id']}}" name="id">
                        <div class="row m-2">
                            ...
                            <button class="btn btn-danger _effect--ripple waves-effect waves-light error_btn mx-2 bs-tooltip" data-bs-placement="bottom" title="Error" data-bs-toggle="modal" data-bs-target="#ErrorListModalCenter">Error</button>
                                <a href="{{route('race')}}" class="btn btn-info _effect--ripple waves-effect waves-light bs-tooltip" data-bs-placement="bottom" title="Back" type="submit">Back</a>
            ...
            <div class="col-lg-12 layout-spacing">
                <div class="statbox widget box box-shadow">
                    <div class="widget-header">
                    <h4>Edit Race Group</h4>
                    </div>
                    <div class="widget-content widget-content-area">

                            <div class="row g-3">
                            <div class="col-md-3">
                                <label for="validationDefault01" class="form-label">Race Group Id<span class="text-danger">
                                        *</span></label>
                                <input type="text" name='GMRAHRaceId' id='GMRAHRaceId' class='form-control threshold' maxlength="20" placeholder="Enter Race Group Name" style='border-color: rgb(102, 175, 233); outline: 0px' value="{{ old('GMRAHRaceId', $raceGroup['GMRAHRaceId'] ?? '') }}" readonly="readonly">
                            </div>
                            <div class="col-md-6">
                                <label for="GMRAHDesc1" class="form-label">Description 1<span class="text-danger"> *</span></label>
                                <input type="text" name='GMRAHDesc1' id='GMRAHDesc1' class='form-control threshold' maxlength="20" placeholder="Enter Description 1" style='border-color: rgb(102, 175, 233); outline: 0px' value="{{ old('GMRAHDesc1', $raceGroup['GMRAHDesc1'] ?? '') }}">

                            </div>
                            <div class="col-md-3">
                                <label for="validationDefault05" class="form-label">BI Desc<span class="text-danger">
                                        *</span></label>
                                <input type="text" name='GMRAHBiDesc' id='GMRAHBiDesc' class='form-control threshold' maxlength="20" placeholder="Enter Bi Desc" style='border-color: rgb(102, 175, 233); outline: 0px' value="{{ old('GMRAHBiDesc', $raceGroup['GMRAHBiDesc'] ?? '') }}">
                            </div>
                            <div class="col-md-12">
                                <label for="validationDefault03" class="form-label">Description 2</label>
                                <textarea name='GMRAHDesc2' id='GMRAHDesc2' class='form-control textarea' maxlength="200" placeholder="Enter Description 2" style='border-color: rgb(102, 175, 233); outline: 0px'>{{ old('GMRAHDesc2', $raceGroup['GMRAHDesc2'] ?? '') }}</textarea>
                            </div>
...
<script type="text/javascript">
    ...
    $("#AddForm").submit(function(e) {
        e.preventDefault();
        $('#GMRAHRaceId,#GMRAHDesc1').removeClass('border border-danger');

        if ($('#GMRAHRaceId').val() == '') {
            $('#GMRAHRaceId').addClass('border border-danger');
        }
        if ($('#GMRAHDesc1').val() == '') {
            $('#GMRAHDesc1').addClass('border border-danger');
        }

        $('.form_error_list').html('');
        if ($('#GMRAHRaceId').val() == '' || $('#GMRAHDesc1').val() == '') {
            if ($('#GMRAHRaceId').val() == '') {
                $('.form_error_list').append('<p>Please Enter Race Group name !</p>');
            }
            if ($('#GMRAHDesc1').val() == '') {
                $('.form_error_list').append('<p>Please Enter Race Group Description 1 !</p>');
            }
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status == 'success') {
                        custom_notification({
                            text: 'Race Group Update successfully',
                            pos: 'bottom-right',
                            duration: 100000
                        })
                    ...
                    if (response.status == 'error') {
                        // Bottom Right
                        custom_notification({
                            text: 'Race Group Master not save',
                            pos: 'bottom-right'
                        })
                    }
                }
