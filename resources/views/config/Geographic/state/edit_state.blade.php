                     ...
                     <input type="hidden" value="{{$state['id']}}" name="id">
                        <div class="row m-2">
                            ...
                            <button class="btn btn-danger _effect--ripple waves-effect waves-light error_btn mx-2 bs-tooltip" data-bs-placement="bottom" title="Error" data-bs-toggle="modal" data-bs-target="#ErrorListModalCenter">Error</button>
                                <a href="{{route('state')}}" class="btn btn-info _effect--ripple waves-effect waves-light bs-tooltip" data-bs-placement="bottom" title="Back" type="submit">Back</a>
            ...
            <div class="col-lg-12 layout-spacing">
                <div class="statbox widget box box-shadow">
                    ...
                    <div class="widget-content widget-content-area">
                            ...
                            <div class="row g-3">
                            <div class="col-md-3">
                                <label for="validationDefault01" class="form-label">State Id<span class="text-danger">
                                        *</span></label>
                                <input type="text" name='GMSMHStateId' id='GMSMHStateId' class='form-control threshold' maxlength="20" placeholder="Enter State Name" style='border-color: rgb(102, 175, 233); outline: 0px' value="{{ old('GMSMHStateId', $state['GMSMHStateId'] ?? '') }}" readonly="readonly">
                            </div>
                            <div class="col-md-6">
                                <label for="GMSMHDesc1" class="form-label">Description 1<span class="text-danger"> *</span></label>
                                <input type="text" name='GMSMHDesc1' id='GMSMHDesc1' class='form-control threshold' maxlength="20" placeholder="Enter State Description 1" style='border-color: rgb(102, 175, 233); outline: 0px' value="{{ old('GMSMHDesc1', $state['GMSMHDesc1'] ?? '') }}">

                            </div>
                            <div class="col-md-3">
                                <label for="validationDefault05" class="form-label">BI Desc<span class="text-danger">
                                        *</span></label>
                                <input type="text" name='GMSMHBiDesc' id='GMSMHBiDesc' class='form-control threshold' maxlength="20" placeholder="Enter Bi Desc" style='border-color: rgb(102, 175, 233); outline: 0px' value="{{ old('GMSMHBiDesc', $state['GMSMHBiDesc'] ?? '') }}">
                            </div>
                            <div class="col-md-12">
                                <label for="validationDefault03" class="form-label">Description 2</label>
                                <textarea name='GMSMHDesc2' id='GMSMHDesc2' class='form-control textarea' maxlength="200" placeholder="Enter State Description 2" style='border-color: rgb(102, 175, 233); outline: 0px'>{{ old('GMSMHDesc2', $state['GMSMHDesc2'] ?? '') }}</textarea>
                            </div>
                            <div class="col-md-4">
                                <label for="validationDefault04" class="form-label">Country<span class="text-danger"> *</span></label>
                                <select id='GMSMHCountryId' name='GMSMHCountryId' class="form-select" style="width: 100%;border: 1px solid #68a6ec;">
                                    <option value=''>Select Country</option>
                                    @foreach ($country_list as $country)
                                    @if (!empty($state['GMSMHCountryId']) && $state['GMSMHCountryId'] == $country->GMCMHCountryId)
...
<script type="text/javascript">
    ...
    $("#AddForm").submit(function(e) {
        e.preventDefault();
        $('#GMSMHStateId,#GMSMHDesc1,#GMSMHCountryId').removeClass('border border-danger');
        ...
        if ($('#GMSMHCountryId').val() == '') {
            $('#GMSMHCountryId').addClass('border border-danger');
        }
        $('.form_error_list').html('');
        if ($('#GMSMHStateId').val() == '' || $('#GMSMHDesc1').val() == '' ||
            $('#GMSMHCountryId').val() == '') {
            if ($('#GMSMHStateId').val() == '') {
                $('.form_error_list').append('<p>Please Enter State name !</p>');
            }
            if ($('#GMSMHDesc1').val() == '') {
                $('.form_error_list').append('<p>Please Enter State Description 1 !</p>');
            }
            if ($('#GMSMHCountryId').val() == '') {
                $('.form_error_list').append('<p>Please Select Country !</p>');
            }
