                     ...
                     <input type="hidden" value="{{$department['id']}}" name="id">
                        <div class="row m-2">
                            ...
                            <button class="btn btn-danger _effect--ripple waves-effect waves-light error_btn mx-2 bs-tooltip" data-bs-placement="bottom" title="Error" data-bs-toggle="modal" data-bs-target="#ErrorListModalCenter">Error</button>
                                <a href="{{route('department')}}" class="btn btn-info _effect--ripple waves-effect waves-light bs-tooltip" data-bs-placement="bottom" title="Back" type="submit">Back</a>
            ...
            <div class="col-lg-12 layout-spacing">
                <div class="statbox widget box box-shadow">
                    ...
                    <div class="widget-content widget-content-area">
                            ...
                            <div class="row g-3">
                            <div class="col-md-3">
                                <label for="validationDefault01" class="form-label">Department Id<span class="text-danger">
                                        *</span></label>
                                <input type="text" name='ESDPHDepartmentId' id='ESDPHDepartmentId' class='form-control threshold' maxlength="20" placeholder="Enter Department Name" style='border-color: rgb(102, 175, 233); outline: 0px' value="{{ old('ESDPHDepartmentId', $department['ESDPHDepartmentId'] ?? '') }}" readonly="readonly">
                            </div>
                            <div class="col-md-6">
                                <label for="ESDPHDesc1" class="form-label">Description 1<span class="text-danger"> *</span></label>
                                <input type="text" name='ESDPHDesc1' id='ESDPHDesc1' class='form-control threshold' maxlength="20" placeholder="Enter Department Description 1" style='border-color: rgb(102, 175, 233); outline: 0px' value="{{ old('ESDPHDesc1', $department['ESDPHDesc1'] ?? '') }}">

                            </div>
                            <div class="col-md-3">
                                <label for="validationDefault05" class="form-label">BI Desc<span class="text-danger">
                                        *</span></label>
                                <input type="text" name='ESDPHBiDesc' id='ESDPHBiDesc' class='form-control threshold' maxlength="20" placeholder="Enter Bi Desc" style='border-color: rgb(102, 175, 233); outline: 0px' value="{{ old('ESDPHBiDesc', $department['ESDPHBiDesc'] ?? '') }}">
                            </div>
                            <div class="col-md-12">
                                <label for="validationDefault03" class="form-label">Description 2</label>
                                <textarea name='ESDPHDesc2' id='ESDPHDesc2' class='form-control textarea' maxlength="200" placeholder="Enter Department Description 2" style='border-color: rgb(102, 175, 233); outline: 0px'>{{ old('ESDPHDesc2', $department['ESDPHDesc2'] ?? '') }}</textarea>
                            </div>
...
<script type="text/javascript">
    ...
    $("#AddForm").submit(function(e) {
        e.preventDefault();
        $('#ESDPHDepartmentId,#ESDPHDesc1').removeClass('border border-danger');

        if ($('#ESDPHDepartmentId').val() == '') {
            $('#ESDPHDepartmentId').addClass('border border-danger');
        }
        if ($('#ESDPHDesc1').val() == '') {
            $('#ESDPHDesc1').addClass('border border-danger');
        }

        $('.form_error_list').html('');
        if ($('#ESDPHDepartmentId').val() == '' || $('#ESDPHDesc1').val() == '') {
            if ($('#ESDPHDepartmentId').val() == '') {
                $('.form_error_list').append('<p>Please Enter Department name !</p>');
            }
            if ($('#ESDPHDesc1').val() == '') {
                $('.form_error_list').append('<p>Please Enter Department Description 1 !</p>');
            }
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status == 'success') {
                        custom_notification({
                            text: 'DepartmentUpdate successfully',
                            pos: 'bottom-right',
                            duration: 100000
                        })
                    ...
                    if (response.status == 'error') {
                        // Bottom Right
                        custom_notification({
                            text: 'DepartmentMaster not save',
                            pos: 'bottom-right'
                        })
                    }
                }
