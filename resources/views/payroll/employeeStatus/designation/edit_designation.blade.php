                     ...
                     <input type="hidden" value="{{$designation['id']}}" name="id">
                        <div class="row m-2">
                            ...
                            <button class="btn btn-danger _effect--ripple waves-effect waves-light error_btn mx-2 bs-tooltip" data-bs-placement="bottom" title="Error" data-bs-toggle="modal" data-bs-target="#ErrorListModalCenter">Error</button>
                                <a href="{{route('designation')}}" class="btn btn-info _effect--ripple waves-effect waves-light bs-tooltip" data-bs-placement="bottom" title="Back" type="submit">Back</a>
            ...
            <div class="col-lg-12 layout-spacing">
                <div class="statbox widget box box-shadow">
                    ...
                    <div class="widget-content widget-content-area">
                            ...
                            <div class="row g-3">
                            <div class="col-md-3">
                                <label for="validationDefault01" class="form-label">Designation Id<span class="text-danger">
                                        *</span></label>
                                <input type="text" name='ESDEHDesignationId' id='ESDEHDesignationId' class='form-control threshold' maxlength="20" placeholder="Enter Designation Name" style='border-color: rgb(102, 175, 233); outline: 0px' value="{{ old('ESDEHDesignationId', $designation['ESDEHDesignationId'] ?? '') }}" readonly="readonly">
                            </div>
                            <div class="col-md-6">
                                <label for="ESDEHDesc1" class="form-label">Description 1<span class="text-danger"> *</span></label>
                                <input type="text" name='ESDEHDesc1' id='ESDEHDesc1' class='form-control threshold' maxlength="20" placeholder="Enter Designation Description 1" style='border-color: rgb(102, 175, 233); outline: 0px' value="{{ old('ESDEHDesc1', $designation['ESDEHDesc1'] ?? '') }}">

                            </div>
                            <div class="col-md-3">
                                <label for="validationDefault05" class="form-label">BI Desc<span class="text-danger">
                                        *</span></label>
                                <input type="text" name='ESDEHBiDesc' id='ESDEHBiDesc' class='form-control threshold' maxlength="20" placeholder="Enter Bi Desc" style='border-color: rgb(102, 175, 233); outline: 0px' value="{{ old('ESDEHBiDesc', $designation['ESDEHBiDesc'] ?? '') }}">
                            </div>
                            <div class="col-md-12">
                                <label for="validationDefault03" class="form-label">Description 2</label>
                                <textarea name='ESDEHDesc2' id='ESDEHDesc2' class='form-control textarea' maxlength="200" placeholder="Enter Designation Description 2" style='border-color: rgb(102, 175, 233); outline: 0px'>{{ old('ESDEHDesc2', $designation['ESDEHDesc2'] ?? '') }}</textarea>
                            </div>
...
<script type="text/javascript">
    ...
    $("#AddForm").submit(function(e) {
        e.preventDefault();
        $('#ESDEHDesignationId,#ESDEHDesc1').removeClass('border border-danger');

        if ($('#ESDEHDesignationId').val() == '') {
            $('#ESDEHDesignationId').addClass('border border-danger');
        }
        if ($('#ESDEHDesc1').val() == '') {
            $('#ESDEHDesc1').addClass('border border-danger');
        }

        $('.form_error_list').html('');
        if ($('#ESDEHDesignationId').val() == '' || $('#ESDEHDesc1').val() == '') {
            if ($('#ESDEHDesignationId').val() == '') {
                $('.form_error_list').append('<p>Please Enter Designation name !</p>');
            }
            if ($('#ESDEHDesc1').val() == '') {
                $('.form_error_list').append('<p>Please Enter Designation Description 1 !</p>');
            }
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status == 'success') {
                        ...
                        window.location = "{{ url('designation') }}";

                    }
                    if (response.status == 'error') {
                        // Bottom Right
                        custom_notification({
                            text: 'Designation Master not save',
                            pos: 'bottom-right'
                        })
                    }
                }
