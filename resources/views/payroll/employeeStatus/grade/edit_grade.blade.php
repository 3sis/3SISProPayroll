                     ...
                     <input type="hidden" value="{{$grade['id']}}" name="id">
                        <div class="row m-2">
                            ...
                            <button class="btn btn-danger _effect--ripple waves-effect waves-light error_btn mx-2 bs-tooltip" data-bs-placement="bottom" title="Error" data-bs-toggle="modal" data-bs-target="#ErrorListModalCenter">Error</button>
                                <a href="{{route('grade')}}" class="btn btn-info _effect--ripple waves-effect waves-light bs-tooltip" data-bs-placement="bottom" title="Back" type="submit">Back</a>
            ...
            <div class="col-lg-12 layout-spacing">
                <div class="statbox widget box box-shadow">
                    ...
                    <div class="widget-content widget-content-area">
                            ...
                            <div class="row g-3">
                            <div class="col-md-3">
                                <label for="validationDefault01" class="form-label">Grade Id<span class="text-danger">
                                        *</span></label>
                                <input type="text" name='ESGRHGradeId' id='ESGRHGradeId' class='form-control threshold' maxlength="20" placeholder="Enter Grade Name" style='border-color: rgb(102, 175, 233); outline: 0px' value="{{ old('ESGRHGradeId', $grade['ESGRHGradeId'] ?? '') }}" readonly="readonly">
                            </div>
                            <div class="col-md-6">
                                <label for="ESGRHDesc1" class="form-label">Description 1<span class="text-danger"> *</span></label>
                                <input type="text" name='ESGRHDesc1' id='ESGRHDesc1' class='form-control threshold' maxlength="20" placeholder="Enter Grade Description 1" style='border-color: rgb(102, 175, 233); outline: 0px' value="{{ old('ESGRHDesc1', $grade['ESGRHDesc1'] ?? '') }}">

                            </div>
                            <div class="col-md-3">
                                <label for="validationDefault05" class="form-label">BI Desc<span class="text-danger">
                                        *</span></label>
                                <input type="text" name='ESGRHBiDesc' id='ESGRHBiDesc' class='form-control threshold' maxlength="20" placeholder="Enter Bi Desc" style='border-color: rgb(102, 175, 233); outline: 0px' value="{{ old('ESGRHBiDesc', $grade['ESGRHBiDesc'] ?? '') }}">
                            </div>
                            <div class="col-md-12">
                                <label for="validationDefault03" class="form-label">Description 2</label>
                                <textarea name='ESGRHDesc2' id='ESGRHDesc2' class='form-control textarea' maxlength="200" placeholder="Enter Grade Description 2" style='border-color: rgb(102, 175, 233); outline: 0px'>{{ old('ESGRHDesc2', $grade['ESGRHDesc2'] ?? '') }}</textarea>
                            </div>
...
<script type="text/javascript">
    ...
    $("#AddForm").submit(function(e) {
        e.preventDefault();
        $('#ESGRHGradeId,#ESGRHDesc1').removeClass('border border-danger');

        if ($('#ESGRHGradeId').val() == '') {
            $('#ESGRHGradeId').addClass('border border-danger');
        }
        if ($('#ESGRHDesc1').val() == '') {
            $('#ESGRHDesc1').addClass('border border-danger');
        }

        $('.form_error_list').html('');
        if ($('#ESGRHGradeId').val() == '' || $('#ESGRHDesc1').val() == '') {
            if ($('#ESGRHGradeId').val() == '') {
                $('.form_error_list').append('<p>Please Enter Grade name !</p>');
            }
            if ($('#ESGRHDesc1').val() == '') {
                $('.form_error_list').append('<p>Please Enter Grade Description 1 !</p>');
            }
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status == 'success') {
                        custom_notification({
                            text: 'Grade Update successfully',
                            pos: 'bottom-right',
                            duration: 100000
                        })
                        window.location = "{{ url('grade') }}";

                    }
                    if (response.status == 'error') {
                        // Bottom Right
                        custom_notification({
                            text: 'Grade Master not save',
                            pos: 'bottom-right'
                        })
                    }
                }
