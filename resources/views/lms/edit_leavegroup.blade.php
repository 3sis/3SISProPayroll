@extends('layouts.app')
@section('page_title','Edit Leave Group')
@section('content')
@section('css')
    ...
    <div class="container">
        <div class="row layout-top-spacing">
            <div id="basic" class="col-lg-12 layout-spacing">
                <div class="statbox widget box box-shadow">
                    ...
                    <form id='AddForm' method="post" autocomplete="off">
                     @csrf
                     <input type="hidden" value="{{$leaveGroup['id']}}" name="id">
                        <div class="row m-2">
                            ...
                            <button class="btn btn-danger _effect--ripple waves-effect waves-light error_btn mx-2 bs-tooltip" data-bs-placement="bottom" title="Error" data-bs-toggle="modal" data-bs-target="#ErrorListModalCenter">Error</button>
                                <a href="{{route('leavegroup')}}" class="btn btn-info _effect--ripple waves-effect waves-light bs-tooltip" data-bs-placement="bottom" title="Back" type="submit">Back</a>
            ...
            <div class="col-lg-12 layout-spacing">
                <div class="statbox widget box box-shadow">
                    <div class="widget-header">
                    <h4>Edit Leave Group</h4>
                    </div>
                    <div class="widget-content widget-content-area">

                            <div class="row g-3">
                            <div class="col-md-3">
                                <label for="validationDefault01" class="form-label">Leave Group Id<span class="text-danger">
                                        *</span></label>
                                <input type="text" name='LMLGHGroupId' id='LMLGHGroupId' class='form-control threshold' maxlength="20" placeholder="Enter Leave Group Name" style='border-color: rgb(102, 175, 233); outline: 0px' value="{{ old('LMLGHGroupId', $leaveGroup['LMLGHGroupId'] ?? '') }}" readonly="readonly">
                            </div>
                            <div class="col-md-6">
                                <label for="LMLGHDesc1" class="form-label">Description 1<span class="text-danger"> *</span></label>
                                <input type="text" name='LMLGHDesc1' id='LMLGHDesc1' class='form-control threshold' maxlength="20" placeholder="Enter Leave Group Description 1" style='border-color: rgb(102, 175, 233); outline: 0px' value="{{ old('LMLGHDesc1', $leaveGroup['LMLGHDesc1'] ?? '') }}">

                            </div>
                            <div class="col-md-3">
                                <label for="validationDefault05" class="form-label">BI Desc<span class="text-danger">
                                        *</span></label>
                                <input type="text" name='LMLGHBiDesc' id='LMLGHBiDesc' class='form-control threshold' maxlength="20" placeholder="Enter Bi Desc" style='border-color: rgb(102, 175, 233); outline: 0px' value="{{ old('LMLGHBiDesc', $leaveGroup['LMLGHBiDesc'] ?? '') }}">
                            </div>
                            <div class="col-md-12">
                                <label for="validationDefault03" class="form-label">Description 2</label>
                                <textarea name='LMLGHDesc2' id='LMLGHDesc2' class='form-control textarea' maxlength="200" placeholder="Enter Leave Group Description 2" style='border-color: rgb(102, 175, 233); outline: 0px'>{{ old('LMLGHDesc2', $leaveGroup['LMLGHDesc2'] ?? '') }}</textarea>
                            </div>
...
<script type="text/javascript">
    ...
    $("#AddForm").submit(function(e) {
        e.preventDefault();
        $('#LMLGHGroupId,#LMLGHDesc1').removeClass('border border-danger');

        if ($('#LMLGHGroupId').val() == '') {
            $('#LMLGHGroupId').addClass('border border-danger');
        }
        if ($('#LMLGHDesc1').val() == '') {
            $('#LMLGHDesc1').addClass('border border-danger');
        }

        $('.form_error_list').html('');
        if ($('#LMLGHGroupId').val() == '' || $('#LMLGHDesc1').val() == '') {
            if ($('#LMLGHGroupId').val() == '') {
                $('.form_error_list').append('<p>Please Enter Leave Group name !</p>');
            }
            if ($('#LMLGHDesc1').val() == '') {
                $('.form_error_list').append('<p>Please Enter Leave Group Description 1 !</p>');
            }
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status == 'success') {
                        custom_notification({
                            text: 'Leave Group Update successfully',
                            pos: 'bottom-right',
                            duration: 100000
                        })
                        window.location = "{{ url('leavegroup') }}";

                    }
                    if (response.status == 'error') {
                        // Bottom Right
                        custom_notification({
                            text: 'Leave Group Master not save',
                            pos: 'bottom-right'
                        })
                    }
                }
